...
class BuilderController extends ChangeNotifier {
  ...
  List<BuilderPlaybackVisualSegment> _playbackVisualSegments =
      const <BuilderPlaybackVisualSegment>[];
  ...
  List<BuilderPlaybackVisualSegment> get playbackVisualSegments =>
      _playbackVisualSegments;

  String get playerInitialDirection {
    final rawDirection = _playerStartEntity?.config['direction']?.toString();
    return rawDirection == null
        ? _pendingPlayerInitialDirection
        : _normalizeFacingDirection(rawDirection);
  }

  String get playerCharacterId {
    final rawCharacter = _playerStartEntity?.config['character']?.toString();
    return rawCharacter == null
        ? _pendingPlayerCharacterId
        : builderCharacterById(rawCharacter).id;
  }
  ...
  void setPlayerInitialDirection(String direction) {
    ...
    if (playerInitialDirection == normalizedDirection || isPlaybackRunning) {
      return;
    }

    _clearPlaybackPreview();
    _pendingPlayerInitialDirection = normalizedDirection;
  ...
  void setPlayerCharacter(String characterId) {
    final normalizedCharacterId = builderCharacterById(characterId).id;
    final playerStart = _playerStartEntity;
    if (playerCharacterId == normalizedCharacterId || isPlaybackRunning) {
      return;
    }

    _clearPlaybackPreview();
    _pendingPlayerCharacterId = normalizedCharacterId;
    ...
    final updatedEntities = project.entities.map((entity) {
      ...
      return entity.copyWith(
        config: <String, dynamic>{
          ...entity.config,
          'character': normalizedCharacterId,
        },
      );
    }).toList();
  ...
  void setCollectableItem(String collectableId) {
    final normalizedCollectableId = builderCollectableById(collectableId).id;
    if (collectableItemId == normalizedCollectableId || isPlaybackRunning) {
      return;
    }
  ...
  void playSolution() {
    ...
    _playbackRunId += 1;
    _playbackVisualSegments = _buildPlaybackVisualSegments(
      steps: playbackPlan.steps,
      initialFacingDirection: playerInitialDirection,
    );
    playbackState = BuilderPlaybackState(
      ...
      facingDirection: playerInitialDirection,
      ...
      if (response['success'] == true) {
        ...
        project = _normalizeProject(
          loadedProject.copyWith(
            ...
            builderType:
                data['builderType']?.toString() ?? loadedProject.builderType,
            courseId: data['courseId']?.toString() ?? loadedProject.courseId,
            orderInCourse:
                _readInt(data['orderInCourse']) ?? loadedProject.orderInCourse,
            difficulty:
      ...
      if (useAdminLevelApi) {
        final response = await ApiService.updateAdminLevel(
          ...
          levelJson: {
            ...
            'orderInCourse': project.orderInCourse,
  ...
  BuilderProject _normalizeProject(BuilderProject input) {
    ...
    final normalizedEntities = input.entities.map((entity) {
      final normalizedType = _normalizeEntityType(entity.type);
      if (normalizedType == 'collectable') {
        return entity.copyWith(
          type: normalizedType,
          config: <String, dynamic>{
            ...entity.config,
      ...
      if (normalizedType != 'playerStart') {
        return entity.copyWith(type: normalizedType);
      }

      final normalizedConfig = <String, dynamic>{
        ...entity.config,
        'direction': _normalizeFacingDirection(
          entity.config['direction']?.toString(),
        ),
        'character': builderCharacterById(
          entity.config['character']?.toString(),
        ).id,
      };
      _pendingPlayerInitialDirection = normalizedConfig['direction'].toString();
      _pendingPlayerCharacterId = normalizedConfig['character'].toString();
      return entity.copyWith(type: normalizedType, config: normalizedConfig);
    }).toList();
  ...
  void _runNextSolutionStep() {
    ...
    if (nextStepIndex >= currentPlaybackPlan.steps.length) {
      ...
      _finishPlayback(
        nextState: currentPlayback.copyWith(
          ...
          facingDirection: currentPlayback.facingDirection,
          animatedCommand: null,
          isPlaying: false,
    ...
    playbackState = currentPlayback.copyWith(
      ...
      facingDirection: _facingDirectionAfterCommand(
        command: nextStep.command,
        previousDirection: currentPlayback.facingDirection,
      ),
      movementStartedAtMs: DateTime.now().millisecondsSinceEpoch,
  ...
  void _scheduleNextPlaybackStep() {
    _playbackTimer?.cancel();
    _playbackTimer = Timer(_playbackStepDuration, _runNextSolutionStep);
  }
  ...
  void _finishPlayback({
    ...
    _playbackTimer = null;
    if (nextState == null) {
      _playbackVisualSegments = const <BuilderPlaybackVisualSegment>[];
      _playbackRunId += 1;
    }
  ...
  List<BuilderPlaybackVisualSegment> _buildPlaybackVisualSegments({
    required List<_PlaybackPlanStep> steps,
    required String initialFacingDirection,
  }) {
    var facingDirection = _normalizeFacingDirection(initialFacingDirection);
    final segments = <BuilderPlaybackVisualSegment>[];
    ...
    for (final step in steps) {
      ...
      segments.add(
        BuilderPlaybackVisualSegment(
          commandId: step.commandId,
          command: step.command,
          fromPlayerX: step.fromPlayerX,
          fromPlayerY: step.fromPlayerY,
          toPlayerX: step.toPlayerX,
          toPlayerY: step.toPlayerY,
          facingDirection: facingDirection,
        ),
      );
    }

    return List<BuilderPlaybackVisualSegment>.unmodifiable(segments);
  }
  ...
  String _facingDirectionAfterCommand({
    required LogicCommandType command,
    required String previousDirection,
  }) {
    switch (command) {
      case LogicCommandType.moveLeft:
      case LogicCommandType.climbUpLeft:
      ...
      case LogicCommandType.jumpUp:
        return _normalizeFacingDirection(previousDirection);
    }
  }

  String _normalizeFacingDirection(String? direction) {
    return direction == playerFacingLeft ? playerFacingLeft : playerFacingRight;
  }
...
class BuilderPlaybackVisualSegment {
  final String commandId;
  final LogicCommandType command;
  final int fromPlayerX;
  final int fromPlayerY;
