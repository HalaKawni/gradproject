...
class DataCoursePage extends StatefulWidget {
  const DataCoursePage({super.key});

  @override
  State<DataCoursePage> createState() => _DataCoursePageState();
}
...
class _DataCoursePageState extends State<DataCoursePage> {
      ...
      setState(() {
        for (int i = 0; i < _lessons.length; i++) {
          ...
          for (final r in results) {
            ...
            if (lvl == (100 + lessonNum) ||
                lvl.toString() == (100 + lessonNum).toString()) {
              quizResult = rMap;
            }
          }
        ...
        _completedLessons = _lessons.where((l) => l.completed == true).length;
        _isLoading = false;
      });
    } catch (e) {
  ...
  void _openLesson(_LessonData lesson) async {
    ...
    final savedSlide = prefs.getInt('data_lesson_slide_${lesson.number}');
    if (!mounted) return;
    ...
    if (lesson.number == 2) {
      ...
      destination = DataLessonPage(lesson: lesson.toMap());
    }
  ...
  Widget _buildLeftPanel() {
    final double progress = _lessons.isEmpty ? 0 : _completedLessons / _lessons.length;
    return Container(
      width: 400,
            ...
            child: Column(
              ...
              children: [
                ...
                Image.asset(lesson.imagePath,
                    width: 400, height: 220, fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => Container(
                        width: 400, height: 220, color: const Color(0xFFE0F7FA),
                        child: const Icon(Icons.bar_chart, size: 60, color: Color(0xFF4DD0C4)))),
                Padding(
                  padding: const EdgeInsets.fromLTRB(14, 14, 14, 18),
                  child: Center(child: Text(lesson.title,
