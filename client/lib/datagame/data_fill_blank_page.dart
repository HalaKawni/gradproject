...
class _DataFillBlankPageState extends State<DataFillBlankPage>
  ...
  void _initSentences() {
    _sentences = [
      _Sentence([
        _SentencePart.text('Sue wanted to put her '),
        _SentencePart.blank(_Blank(id: 'b1', answer: 'numerical')),
        _SentencePart.text(' data in '),
        _SentencePart.blank(_Blank(id: 'b2', answer: 'ascending')),
        _SentencePart.text(' order, so she started the list with the least value number and ended with the '),
        _SentencePart.blank(_Blank(id: 'b3', answer: 'greatest')),
        _SentencePart.text(' value.'),
      ]),
      _Sentence([
        _SentencePart.text('Calvin likes creating '),
        _SentencePart.blank(_Blank(id: 'b4', answer: 'picture graphs')),
        _SentencePart.text(' because he gets to draw cool images of the data he collected.'),
      ]),
      _Sentence([
        _SentencePart.text('Bar Charts use the '),
        _SentencePart.blank(_Blank(id: 'b5', answer: 'height')),
        _SentencePart.text(' of the columns to display quantity.'),
      ]),
      _Sentence([
        _SentencePart.text('A '),
        _SentencePart.blank(_Blank(id: 'b6', answer: 'table')),
        _SentencePart.text(' is a good way to organize data without graphing it.'),
      ]),
      _Sentence([
        _SentencePart.blank(_Blank(id: 'b7', answer: 'Hash')),
        _SentencePart.text(' marks are a simple way to count how many times a value occurs.'),
      ]),
    ];
  }
        ...
        child: Stack(
          children: [
            ...
            AnimatedBuilder(
              ...
              builder: (_, _) => _confettiCtrl.value > 0
...
class _PondBlankState extends State<_PondBlank>
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isCorrect) {
      deco = BoxDecoration(
        ...
        boxShadow: [
          ...
          BoxShadow(color: _WPColors.mintDeep.withValues(alpha: 0.30), blurRadius: 0, spreadRadius: 3),
        ],
      );
      ...
      deco = BoxDecoration(
        ...
        boxShadow: [
          ...
          BoxShadow(color: _WPColors.coral.withValues(alpha: 0.5), blurRadius: 0, spreadRadius: 4),
        ],
      );
    ...
    final inner = AnimatedContainer(
      ...
      constraints: const BoxConstraints(minWidth: 80, maxWidth: 200, minHeight: 34),
    ...
    return Padding(
      ...
      child: AnimatedBuilder(
        ...
        child: DragTarget<String>(
          ...
          builder: (_, _, _) => GestureDetector(onTap: widget.onTap, child: inner),
        ),
      ),
    );
  }
...
class _ChickPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(24*s,42*s), 3*s,
        Paint()..color = _WPColors.coral.withValues(alpha: 0.55));
    canvas.drawCircle(Offset(56*s,42*s), 3*s,
        Paint()..color = _WPColors.coral.withValues(alpha: 0.55));
  }
  @override
    ...
    return AnimatedBuilder(
      ...
      child: GestureDetector(
        ...
        child: Container(
          width: 64, height: 64,
          decoration: BoxDecoration(
            color: widget.onTap != null ? _WPColors.sun : Colors.grey.withValues(alpha: 0.4),
...
class _RuledLinesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final p = Paint()
      ..color = _WPColors.skyDeep.withValues(alpha: 0.18)
      ..strokeWidth = 1;
    const spacing = 52.0;
...
class _MeadowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawOval(Rect.fromCenter(center: Offset(w*0.15, h+30), width:600, height:180),
        Paint()..color = _WPColors.grass.withValues(alpha: 0.9));
    ...
    canvas.drawCircle(Offset(w-80, 30), 32, Paint()..color = _WPColors.sun.withValues(alpha: 0.85));
...
class _ConfettiOverlayState extends State<_ConfettiOverlay> {
  ...
  void initState() {
    ...
    const colors = [_WPColors.sun, _WPColors.coral, _WPColors.mint,
                    _WPColors.grass, _WPColors.purple];
